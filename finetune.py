...
if __name__ == "__main__":
    ...
    parser.add_argument("--load-ckpt", type=str, default="ckpt.pt")
    parser.add_argument("--save-ckpt", type=str, default="ckpt.pt")
    ...
    print("Loading model from ", args.load_ckpt)
    # shrink model seqlen to load trained model
    model.update_sizes(1)
    model.load_state_dict(torch.load(args.load_ckpt))
    # increase back to train length
    model.update_sizes(args.train_ctx_len)
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1,
                                  drop_last=True)
    ...
    np.savez_compressed(args.save_ckpt.replace(".pt", "_2048_losses"), losses)
